  ...
  describe "GET /favorite_food_ids" do
    context "正しい情報でリクエストしたとき" do
      ...
      it "期待するデータが含まれていること" do
        expect(JSON.parse(response.body)).to have_key("food_ids")
        expect(JSON.parse(response.body)["food_ids"]).to eq([food.id])
      end
    end
  ...
  describe "POST /favorites" do
    ...
    context "正しい情報でリクエストしたとき" do
      ...
      it "期待するフードがお気に入り登録されていること" do
        ...
        expect(JSON.parse(response.body)["favorite"]["food_id"]).to eq(other_food.id)
      end
    end
  ...
  describe "DELETE /favorites" do
    context "正しい情報でリクエストしたとき" do
      ...
      it "期待するフードがお気に入りから削除されていること" do
        expect(JSON.parse(response.body)["favorite"]["user_id"]).to eq(user.id)
        expect(JSON.parse(response.body)["favorite"]["food_id"]).to eq(food.id)
      end
    end
    ...
    context "他のログイン状態のユーザーがリクエストしたとき" do
      ...
      it "返されるエラーメッセージが期待どおりであること" do
        expect(JSON.parse(response.body)["error"]).to include("お気に入りが見つかりません")
      end
    end
